...
import { Users, BookOpen, Calendar, Award, TrendingUp, MessageSquare, FileText, CheckCircle, Clock, Target, Video, Briefcase, User } from 'lucide-react';
import { supabase } from '@/lib/supabase';
import { useToast } from '@/hooks/use-toast';
import { formatDistanceToNow } from 'date-fns';
import { InstructorGradingUI } from '@/components/InstructorGradingUI';
...
const BootcampDashboardPage = () => {
  ...
  // Mock enrollment for testing/instructor view if none exists
  const effectiveEnrollment = enrollment || {
    id: 'demo-enrollment',
    cohort: {
      id: 'demo-cohort',
      name: 'Demo Cohort (Instructor View)',
      start_date: new Date().toISOString(),
      end_date: new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString(),
    },
    progress_percentage: 0,
    pre_bootcamp_apo_score: null,
    post_bootcamp_apo_score: null,
  ...
  // Removed the blocking check for enrollment to allow instructor access
  // if (!enrollment) { ... }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">{effectiveEnrollment.cohort.name}</h1>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{effectiveEnrollment.progress_percentage}%</p>
              </div>
            </div>
            <Progress value={effectiveEnrollment.progress_percentage} className="mt-4" />
          </CardContent>
        </Card>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {effectiveEnrollment.pre_bootcamp_apo_score && effectiveEnrollment.post_bootcamp_apo_score
                    ? `-${(effectiveEnrollment.pre_bootcamp_apo_score - effectiveEnrollment.post_bootcamp_apo_score).toFixed(0)} pts`
                    : 'TBD'}
                </p>
              </div>
            </div>
