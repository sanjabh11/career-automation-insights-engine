...
function buildCorsHeaders(origin: string): Record<string, string> {
  ...
  return { ...baseCorsHeaders, 'Access-Control-Allow-Origin': allowOrigin } as Record<string, string>;
}
...
async function handler(req: Request) {
  ...
  const requiredKey = (Deno.env.get('ECON_SYNC_API_KEY') || '').replace(/\s+/g, '');
  if (requiredKey) {
    const provided = (req.headers.get('x-api-key') || '').replace(/\s+/g, '');
  ...
  try {
    ...
    for (const r of rows) {
      ...
      const cat = (String(r?.task_category ?? (hasAdoption ? 'Adoption' : (hasPayback ? 'Payback' : (hasMaturity ? 'Maturity' : 'General')))).trim() || 'General');
      r.task_category = cat;

      const k1 = cat.toLowerCase();
      const k2 = String(r.industry_sector ?? '').trim().toLowerCase();
      const k3 = String(r.as_of_year ?? '').trim();
      const k4 = String(r.region ?? '').trim().toLowerCase();
      const k5 = String(r.country ?? '').trim().toLowerCase();
      const key = `${k1}|${k2}|${k3}|${k4}|${k5}`;
      deduped.set(key, r);
    }
    ...
    const { error } = await supabase
      ...
      .upsert(mapped, { onConflict: 'task_category,industry_sector,as_of_year,region,country' });
    if (error) {
      return new Response(JSON.stringify({ error: error.message }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    return new Response(JSON.stringify({ ok: true, totalUpserted: mapped.length, dedupedFrom: rows.length }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error) {
