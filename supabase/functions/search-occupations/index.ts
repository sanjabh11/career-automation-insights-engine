...
export async function handler(req: Request) {
  ...
  const cors = {
    ...BASE_CORS_HEADERS,
    // For localhost we always echo the origin to simplify local development
  ...
  if (req.method === "OPTIONS") {
    // In dev, always allow localhost preflight to succeed
    if (!isLocalhost && !allowAll && origin && !allowedOrigins.includes(origin)) {
