-- ROI calculation function for occupations
-- Computes a conservative ROI months estimate from sector economics and wage
...
BEGIN
  ...
  IF v_soc6 IS NULL THEN v_soc6 := p_soc8; END IF;

  SELECT median_wage_annual INTO v_wage
  FROM public.bls_employment_data
  WHERE occupation_code_6 = v_soc6
  ORDER BY year DESC
  LIMIT 1;

  SELECT CASE career_cluster
  ...
  SELECT avg(implementation_cost_low), avg(implementation_cost_high)
  INTO v_cost_low, v_cost_high
  FROM public.automation_economics
  WHERE industry_sector = v_sector;
  ...
  RETURN QUERY SELECT p_soc8, v_sector, v_wage, v_avg_cost, v_roi_months;
END $$;
