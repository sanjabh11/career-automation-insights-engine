...
| Owner evidence prep | `npm run verify:owner-evidence-prep`, `npm run prepare:owner-evidence -- --write` | Inspect or create gitignored local owner-evidence scaffolding without printing secrets or making placeholders count as proof. |
...
| Owner evidence closeout | `npm run verify:owner-evidence-closeout`, `npm run closeout:owner-evidence -- --write --refresh-tracked` | Run the ordered owner-proof closeout bundle: compose redacted evidence, validate live/commercial records, fail closed on the final remediation gates, and refresh tracked ledgers only when evidence is complete. |
