...
FUNCTION_PATH = "/functions/v1/calculate-apo"
DEFAULT_QUEUE_FILE = "apo_queue.csv"
DEFAULT_BATCH_SIZE = 10
DEFAULT_START_RANK = 1
REQUEST_TIMEOUT_SECONDS = 180
SLEEP_SECONDS = 2
REQUIRED_COLUMNS = {"priority_rank", "occupation_code_8", "occupation_title"}
...
def resolve_queue_path(raw_path: str) -> Path:
    ...
    fail(
        ...
        "2. Export the 'TOP APO QUEUE' result set as CSV\n"
        "3. Save as apo_queue.csv"
    )
    return queue_path
    ...
    with queue_path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        if "priority_rank" not in field_set and SUMMARY_COLUMNS.issubset(field_set):
            fail(
                ...
                "Export 'TOP APO QUEUE', not the batch summary result set."
            )
...
def build_headers() -> dict[str, str]:
    ...
    api_key = os.environ.get("APO_FUNCTION_API_KEY", "").strip()
    if api_key:
...
def main(argv: list[str]) -> int:
    ...
    log_path = Path(__file__).resolve().parent / f"apo_backfill_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"

    print("===================================")
    print("APO Backfill Runner")
    print("===================================")
    ...
    if "x-api-key" in headers:
        print("Using APO_FUNCTION_API_KEY for authentication")
    else:
        print("No APO_FUNCTION_API_KEY set; using anon key only")
    print()
    ...
    for row in rows:
        ...
        if not occupation_title:
            fail(f"ERROR: Row {priority_rank} is missing occupation_title")
    ...
    print(f"4. Process next batch with: ./scripts/backfill-apo.sh {queue_file} {batch_size} {end_rank + 1}")
    return 0
