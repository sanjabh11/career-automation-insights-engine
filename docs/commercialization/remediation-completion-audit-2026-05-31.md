...
| Full lint | `npm run lint` from Phase E follow-up gate run | Fail on active type-hardening debt: 21 explicit-`any` errors and 31 React warnings after inactive backup/archive trees, mechanical non-`any` errors, and runtime env bootstrap, Netlify APO/O*NET proxy event boundary `any` casts, Career Planning job-market response `any` casts, header validation-metric query `any` casts, dashboard saved-analysis load contract `any` casts, assistant function response/error handling `any` casts, outcomes list row/error `any` casts, selected-career prop `any` casts, organization/search-history/subscription hook error and input `any` boundaries, econ sync runtime env row `any` casts, API error normalization `any` casts, web-vitals compatibility/persistence `any` casts, stat-card icon `any` typing, AI Impact Dashboard search parsing, Career Simulator result/risk handling, AI Impact Planner, Outcomes KPI, calculate-APO, Industry dashboard, Validation calibration, Job Zones, Job Zone Ladders query row `any` casts, Quality web vitals query `any` casts, Technology Skills query result `any` casts, course-search SerpAPI result `any` casts, counselor report HTML contract `any` casts, APO UI selection/explanation boundary `any` casts, Skill Adjacency Graph, O*NET enrichment, Whop webhook, roadmap-generation, task-analysis, BLS sync, heatmap snapshot, skill recommendation, intelligent task-assessment test, personalized skill-recommendation test, Bridge Role Pathway, Ecosystem Risk Card, Premium Report Summary, O*NET enrichment hook, saved-analyses unified hook, saved-analyses remote hook, Occupation Detail, Operations telemetry, explain-APO, find-bridge-roles, generate-learning-path, search-tasks, manage-context, search-occupations, serpapi-jobs, useSkills error `any` casts, Crosswalk Wizard payload `any` casts, Outcome Survey payload/form `any` casts, useAlerts error `any` casts, useSavedSelections JSON/error `any` casts, Bootcamp Dashboard query `any` casts, Compare Page saved-analysis item `any` casts, Saved Analyses Panel callback/share `any` casts, Market Map tooltip/invoke `any` casts, Shared Analysis RPC payload/error `any` casts, useShareAnalysis payload `any` casts, Workshop Booking select callback `any` casts, AI career coach function/test `any` casts, market-intelligence job-data/analysis `any` casts, shared SerpAPI client result `any` casts, and serpapi-jobs response typing were cleared |
