...
| Full lint | `npm run lint` from Phase E follow-up gate run | Fail on active type-hardening debt: 289 explicit-`any` errors and 34 React warnings after inactive backup/archive trees, mechanical non-`any` errors, and AI Impact Planner, Outcomes KPI, calculate-APO, plus Industry dashboard payload `any` casts were cleared |
