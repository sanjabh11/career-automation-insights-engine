...
| Phase 3: Skill Change Ledger | Reports render growing, stable, declining, changing, unknown states and protect, upgrade, replace, learn-next actions with per-row confidence, review state, source IDs, source caveats, and a dedicated learning/provider boundary that blocks course/vendor/outcome overclaims. | Live labor-market validation, local posting proof, and licensed provider adapters. | 4 | 6% | 5% | Yes for MVP proof pack; continue before provider-backed skill-market claims. |
...
| 3 | Skills show status, action, confidence, review state, source IDs, and caveat; unknown/local-demand rows stay review-required until validated; learning themes stay separate from course/vendor recommendations. |
