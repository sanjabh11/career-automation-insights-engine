...
import { Loader2, Search, ListChecks, Download } from "lucide-react";
...
export default function TaskSearchPage() {
  ...
  const exportCSV = () => {
    try {
      // Flatten tasks across occupations into CSV rows
      ...
      a.href = url;
      a.download = `task_search_${query.replace(/\s+/g, "_")}_${Date.now()}.csv`;
      a.click();
