        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            <Button
              ...
              size="sm"
              variant="outline"
              onClick={() => { setBranch("army"); setMocCode("11B"); setSubmitted(true); }}
            >
              Try 11B (Army Infantry)
            </Button>
            <Button
              type="button"
              size="sm"
              variant="outline"
              onClick={() => { setBranch("army"); setMocCode("68W"); setSubmitted(true); }}
            >
              Try 68W (Army Medic)
            </Button>
            <Button
              ...
              size="sm"
              variant="outline"
              onClick={() => { setBranch("navy"); setMocCode("AE"); setSubmitted(true); }}
            >
              Try AE (Navy Avionics)
            </Button>
          </div>
        </form>
      </Card>
...
function CivilianOccupationCard({ occupation, index, userId }: CivilianOccupationCardProps) {
  const { enrichmentData, isLoading } = useOnetEnrichment(occupation.code);
  const [genLoading, setGenLoading] = React.useState(false);
  const [genOk, setGenOk] = React.useState<string | null>(null);
        ...
        <div className="space-y-3">
          ...
          <div className="pt-2 border-t">
            ...
            {userId && (
              <Button
                ...
                onClick={async () => {
                  try {
                    setGenLoading(true);
                    setGenOk(null);
                    const stub = [{ skill: "Transition Readiness", recommendation: "Bridge military-to-civilian competencies with certification prep." }];
              ...
              >
                {genLoading ? "Generating…" : (genOk || "Generate Learning Path")}
              </Button>
            )}
          </div>
        </div>
