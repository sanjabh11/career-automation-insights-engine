...
ALTER TABLE onet_career_clusters
...
WITH data AS (
  ...
  ) AS t(cluster_id, title, description, sort_order)
)
INSERT INTO onet_career_clusters (cluster_id, name, cluster_title, description, sort_order)
SELECT cluster_id, title, title, description, sort_order FROM data
