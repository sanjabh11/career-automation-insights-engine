...
### v2.1 Improvements (2026-02-08)
- **O*NET Data Grounding**: Real task statements, knowledge, abilities, and technologies are fetched from DB tables (`onet_detailed_tasks`, `onet_knowledge`, `onet_abilities`, `onet_technologies`) and injected into the LLM prompt. The LLM must base its analysis on authoritative O*NET data rather than training-data hallucinations.
- **Result Caching**: Before making an LLM call, the function checks `apo_logs` for a recent result (within `APO_CACHE_TTL_HOURS`, default 24h). Cache hits return instantly with `X-APO-Cache: HIT` header.
- **Confidence-Scaled Monte Carlo CI**: Noise std dev is now inversely scaled by category confidence (high=2%, medium=5%, low=10%) instead of a flat 3%. This produces honest CI bands.
- **Per-Item Timelines**: Each item now includes a `timeline` field (`immediate|short_term|medium_term|long_term`) instead of the previous hardcoded `unknown`.
- **Dead Code Removal**: Removed unused `AUTOMATION_FACTORS` and `calculateWeightedAPO`.
...
```json
{
  ...
  "confidence": "medium",
  "timeline": "short_term",
  "tasks": [{ "description": "...", "apo": 55.2, "factors": ["routine"], "timeline": "short_term" }],
  "knowledge": [...],
  "skills": [...],
  "abilities": [...],
  "technologies": [...],
...
- Items are parsed and validated against a strict Zod schema (includes per-item `timeline` field).
...
## O*NET Data Grounding
Before the LLM call, real O*NET data is fetched from the database:
- `onet_detailed_tasks` — up to 20 task statements ordered by importance
- `onet_knowledge` — up to 15 knowledge areas with importance/level
- `onet_abilities` — up to 15 abilities with importance/level
- `onet_technologies` — up to 20 technologies with hot-tech flags

This data is formatted and appended to the prompt context. When O*NET data is available, the prompt instructs the LLM to base task items on the provided statements rather than generating from training data.

## Caching
- Before the LLM call, `apo_logs` is queried for a recent result matching `occupation_code` (and optionally `config_id`) within `APO_CACHE_TTL_HOURS` (default: 24).
- Cache hits skip the LLM call entirely and return the stored result with `X-APO-Cache: HIT`.
- Set `APO_CACHE_TTL_HOURS=0` to disable caching.
...
- Optional (caching): `APO_CACHE_TTL_HOURS` (default `24`; set to `0` to disable)
