...
- `npm run verify:live-gate-evidence` passes with no attached local evidence file; the adversarial check against `docs/commercialization/live-gate-evidence-template.json` fails as intended because placeholder hashes cannot count as proof, and the verifier now rejects private-contact patterns and future-dated evidence metadata.
