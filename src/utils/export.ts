...
export interface CareerAnalysisData {
  ...
  confidence: string;
  timeline: string;
  categoryBreakdown?: Record<string, { apo: number; confidence: string }>;
  insights?: {
...
export function exportToCSV(jobs: any[], filename = 'career_analyses.csv') {
  const rows: CSVRow[] = (jobs || []).map((job) => ({
    ...
    apo_score: job.overallAPO ?? job.apo_score ?? '',
    confidence: job.confidence || '',
    timeline: job.timeline || '',
  }));
  const csv = toCSV(rows, ['occupation_code', 'occupation_title', 'apo_score', 'confidence', 'timeline']);
  download(filename, csv);
}

export function exportAnalysisToPDF(analyses: CareerAnalysisData[], title = 'Career Automation Analysis Report') {
  const rows = (analyses || []).map((analysis) => `
    <tr>
      <td>${escapeHTML(analysis.title || '')}</td>
      <td>${escapeHTML(analysis.code || '')}</td>
      <td>${escapeHTML(String(analysis.overallAPO ?? ''))}</td>
      <td>${escapeHTML(analysis.confidence || '')}</td>
      <td>${escapeHTML(analysis.timeline || '')}</td>
    </tr>
  `).join('');
