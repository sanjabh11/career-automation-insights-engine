...
export default function EconImporter() {
  ...
  useEffect(() => {
    try {
      ...
      // apiKey intentionally NOT loaded from localStorage (security: avoid persisting secrets)
      ...
      // apiKey hydration removed for security
      if (so) setSource(so);
  ...
  useEffect(() => {
    try {
      ...
      // apiKey intentionally NOT persisted to localStorage (security risk)
  ...
  if (!enabled) {
    return (
      <div className="container mx-auto max-w-2xl p-6">
        ...
        <p className="text-sm text-[var(--text-secondary)]">This importer is disabled in production by default. To enable it, set <code>VITE_ENABLE_ECON_IMPORTER=true</code> in your environment and rebuild. This prevents exposing administrative ingestion tools to end users.</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto max-w-4xl p-4">
      ...
      <div className="flex items-center gap-3 mb-4">
        {!running ? (
          <button className="bg-[var(--accent-primary)] text-white px-4 py-2 rounded" onClick={doImport}>Start Import</button>
        ) : (
          <>
            <button className="bg-slate-700 text-white px-4 py-2 rounded" onClick={onAbort}>Abort</button>
      ...
      <div className="mt-6 text-xs text-[var(--text-secondary)] space-y-2">
