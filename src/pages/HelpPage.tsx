      ...
      <div className="max-w-5xl mx-auto px-4 py-8 md:py-12 space-y-8">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-2">
            <h1 className="text-2xl md:text-3xl font-bold tracking-tight flex items-center gap-2">
              <HelpCircle className="h-6 w-6 text-[var(--accent-primary)]" /> Help & Getting Started
            </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {glossary.map((g) => (
              <div key={g.term} className="border rounded-lg p-4">
                ...
                <div className="text-sm text-[var(--text-secondary)] mt-1">{g.definition}</div>
              </div>
            ))}
          </div>
        </Card>
        ...
        <Card className="p-6">
          ...
          <ul className="space-y-2 text-sm">
            <li>
              <span className="font-mono bg-[var(--bg-tertiary)] px-2 py-0.5 rounded">?</span> Open Help from anywhere
            </li>
          </ul>
        </Card>
