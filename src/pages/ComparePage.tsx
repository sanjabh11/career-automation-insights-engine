...
const ComparePage: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
              ...
              <div key={a.id} className={`border rounded-md p-3 bg-[var(--bg-tertiary)] ${selected.includes(a.id) ? 'ring-2 ring-[var(--accent-primary)]' : ''}`}>
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-[var(--text-secondary)]">{a.occupation_code}</div>
                  </div>
                  <input
                    type="checkbox"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {selectedItems.map((a: SavedAnalysisItem) => (
          <Card key={a.id} className="p-4">
            ...
            {a.notes && (
              <div className="text-sm bg-[var(--bg-secondary)] border rounded p-2 text-[var(--text-secondary)] mb-2">{a.notes}</div>
            )}
      ...
      {selected.length === 0 && (
        <div className="text-sm text-[var(--text-tertiary)] mt-4">Select items above to compare.</div>
      )}
    </div>
  );
};
