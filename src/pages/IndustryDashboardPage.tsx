...
type ClusterDetailResponse = {
  ...
  occupations: ClusterOccupation[];
  occupationCount: number;
  source?: string;
};

type ClusterResponse = ClusterListResponse | ClusterDetailResponse;

const isClusterListResponse = (data: ClusterResponse | undefined): data is ClusterListResponse =>
  Boolean(data && "clusters" in data);

const isClusterDetailResponse = (data: ClusterResponse | undefined): data is ClusterDetailResponse =>
  Boolean(data && "cluster" in data);

export default function IndustryDashboardPage() {
  const [selectedCluster, setSelectedCluster] = useState<string | null>(null);
  const navigate = useNavigate();

  const clustersQuery = useQuery<ClusterResponse>({
    queryKey: ["career-clusters", selectedCluster],
    queryFn: async () => {
      if (!selectedCluster) {
        const { data, error } = await supabase.functions.invoke("browse-career-clusters");
        if (error) throw error;
        return data as ClusterListResponse;
      } else {
        const { data, error } = await supabase.functions.invoke("browse-career-clusters", {
          body: { clusterId: selectedCluster, includeOccupations: true },
        });
        if (error) throw error;
        return data as ClusterDetailResponse;
      }
    },
    staleTime: 60_000,
  });
  ...
  const clusterList = isClusterListResponse(clustersQuery.data) ? clustersQuery.data.clusters : [];
  const clusterDetail = isClusterDetailResponse(clustersQuery.data) ? clustersQuery.data : undefined;
  const source = clustersQuery.data?.source || "db";
      ...
      <Card className="p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div className="text-sm text-muted-foreground">
            {selectedCluster ? "Cluster Details" : `${clusterList.length} clusters`}
          </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            <div className="font-semibold">{clusterDetail?.cluster.cluster_name || selectedCluster}</div>
            <div className="text-sm text-muted-foreground">{clusterDetail?.occupationCount || 0} occupations</div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {clusterDetail?.occupations.map((o) => (
                <Card 
