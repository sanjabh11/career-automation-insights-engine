    ...
    <Card className="p-6 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {occupations.map((occ) => {
          ...
          return (
            <div key={occ.code} className="bg-[var(--bg-tertiary)] rounded-lg p-4 shadow-sm flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium text-base">{occ.title}</p>
                  <span className="text-xs text-[var(--text-tertiary)]">{occ.code}</span>
                </div>
                <button
              ...
              <div className="flex flex-wrap gap-1 text-xs items-center">
                <Badge className="bg-[var(--accent-primary)]/20 text-[var(--accent-primary)]">{occ.timeline}</Badge>
              ...
              <div className="text-3xl font-semibold text-[var(--accent-primary)]">{(occ.overallAPO ?? 0).toFixed(1)}%</div>
            </div>
          );
        })}
      </div>
        ...
        <table className="min-w-full border rounded">
          <thead>
            <tr className="bg-slate-100">
              <th className="py-2 px-2 text-left text-xs font-medium text-[var(--text-secondary)]">Category</th>
              {occupations.map((occ) => (
                <th key={occ.code} className="py-2 px-2 text-xs font-medium text-[var(--text-secondary)]">{occ.title}</th>
              ))}
            </tr>
          </thead>
          <tbody>
