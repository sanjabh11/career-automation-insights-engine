    ...
    <Card className="p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        <div className="rounded border p-3">
          <div className="flex items-center gap-2 mb-1"><DollarSign className="h-4 w-4 text-green-600" /><span className="text-xs text-[var(--text-secondary)]">Investment</span></div>
          <div className="text-lg font-semibold">{invest != null ? `$${Math.round(invest).toLocaleString()}` : '—'}</div>
          <div className="text-[11px] text-[var(--text-tertiary)]">Courses + misc. costs</div>
        </div>
        <div className="rounded border p-3">
          <div className="flex items-center gap-2 mb-1"><Clock className="h-4 w-4 text-[var(--accent-primary)]" /><span className="text-xs text-[var(--text-secondary)]">Payback</span></div>
          <div className="text-lg font-semibold">{months != null ? `${months} months` : '—'}</div>
          <div className="text-[11px] text-[var(--text-tertiary)]">Breakeven timeline</div>
        </div>
        <div className="rounded border p-3">
          <div className="flex items-center gap-2 mb-1"><TrendingUp className="h-4 w-4 text-[var(--accent-primary)]" /><span className="text-xs text-[var(--text-secondary)]">Annual Wage (est.)</span></div>
          <div className="text-lg font-semibold">{typeof roi?.annual_wage === 'number' ? `$${Math.round(roi.annual_wage).toLocaleString()}` : '—'}</div>
          <div className="text-[11px] text-[var(--text-tertiary)]">For current occupation</div>
        </div>
      </div>

      {data ? (
        <div className="bg-[var(--bg-secondary)] rounded border border-[hsl(var(--border))] p-3">
          <div className="text-xs font-medium text-[var(--text-secondary)] mb-2">Cumulative impact (simplified)</div>
          ...
          {months != null && (
            <div className="text-[11px] text-[var(--text-tertiary)] mt-2">Crosses breakeven around month {months}. Synthetic chart when detailed plan data is unavailable.</div>
          )}
        </div>
      ) : (
        <div className="text-[12px] text-[var(--text-secondary)]">Detailed breakeven chart unavailable. Missing either investment or payback months.</div>
      )}
    </Card>
  );
}
