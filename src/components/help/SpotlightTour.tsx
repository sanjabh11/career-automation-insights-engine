...
export function SpotlightTour({ steps, open, onClose }: { steps: TourStep[]; open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[1000]">
      ...
      {/* Tooltip panel */}
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-[var(--bg-secondary)] rounded-lg shadow-xl max-w-md w-[92vw] p-4">
        <div className="text-sm font-semibold mb-1">{step.title}</div>
        <div className="text-xs text-[var(--text-secondary)] mb-3">{step.description}</div>
