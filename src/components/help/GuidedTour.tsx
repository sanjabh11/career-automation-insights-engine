  ...
  return (
    <div className="fixed inset-0 z-[60]">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="pointer-events-auto max-w-md w-[92%] sm:w-[520px] mx-auto mt-24 bg-[var(--bg-secondary)] rounded-xl shadow-2xl p-5">
          <div className="text-sm text-[var(--text-tertiary)]">Step {i + 1} of {steps.length}</div>
          <h3 className="text-lg font-semibold mt-1">{steps[i].title}</h3>
          <p className="text-sm text-[var(--text-secondary)] mt-2">{steps[i].description}</p>
