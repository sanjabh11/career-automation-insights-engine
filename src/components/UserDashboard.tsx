...
function OnboardingHelpOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    ...
    >
      <motion.div 
        className="bg-[var(--bg-secondary)] p-4 sm:p-6 rounded-lg shadow-lg max-w-md w-full relative"
      ...
      >
        <button 
          className="absolute top-3 right-3 text-[var(--text-tertiary)] hover:text-[var(--text-secondary)] text-xl" 
          onClick={onClose} 
        ...
        <ul className="list-disc pl-5 space-y-2 text-sm text-[var(--text-secondary)]">
          ...
          <li>Need support? Click your profile/settings or visit the <span className="underline text-[var(--accent-primary)]">Support</span> tab.</li>
        </ul>
...
export function UserDashboard({ onLoadAnalysis, onSearchSelect }: UserDashboardProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-[var(--text-primary)]">User Dashboard</h1>
          <p className="text-sm sm:text-base text-[var(--text-secondary)]">Manage your profile, settings, and saved data</p>
        </div>
        <motion.button
          className="inline-flex gap-1 items-center text-[var(--accent-primary)] bg-[var(--accent-primary)]/10 hover:bg-[var(--accent-primary)]/20 px-3 py-1.5 rounded-md text-sm transition-colors duration-200 w-fit"
      ...
      >
        <Tabs defaultValue="profile" className="w-full">
          <div className="overflow-x-auto">
            <TabsList className="grid w-full grid-cols-5 min-w-max sm:min-w-0 bg-[var(--bg-secondary)]/80 backdrop-blur-sm border border-[hsl(var(--border))]/50 p-1">
