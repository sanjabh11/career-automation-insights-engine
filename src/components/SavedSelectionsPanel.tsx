  ...
  return (
    <Card className="p-6 mb-6">
      ...
      <div className="mt-4 space-y-2">
        {saved.length === 0 && (
          <p className="text-xs text-[var(--text-tertiary)]">No saved selections yet.</p>
        )}
        {saved.map((s) => (
          <div key={s.id} className="flex items-center gap-2 justify-between bg-slate-50 rounded-md px-3 py-2">
            <div className="flex-1 flex gap-2 items-center">
              ...
              {editingId === s.id ? (
                ...
                <>
                  <span className="font-medium text-[var(--text-secondary)] text-sm">{s.name}</span>
                  <Button
                    size="icon"
                    variant="ghost"
                    className="h-6 w-6"
                  ...
                  >
                    <PencilLine className="w-4 h-4 text-[var(--text-tertiary)]" />
                  </Button>
                </>
              )}
            </div>
