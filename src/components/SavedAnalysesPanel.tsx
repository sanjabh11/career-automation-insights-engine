    ...
    return (
      <Card className="p-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[var(--text-tertiary)]">Loading saved analyses...</span>
        </div>
      </Card>
    );
  }
      ...
      <Card className="p-6">
        ...
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {savedAnalyses.length === 0 ? (
            <p className="text-[var(--text-tertiary)] text-sm">No saved analyses yet.</p>
          ) : (
            savedAnalyses.map((analysis) => (
              <div key={analysis.id} className="border rounded-lg p-4 bg-[var(--bg-tertiary)]">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    <h4 className="font-medium text-sm">{analysis.occupation_title}</h4>
                    <p className="text-xs text-[var(--text-tertiary)]">Code: {analysis.occupation_code}</p>
                  </div>
                ...
                {analysis.notes && (
                  <p className="text-xs text-[var(--text-secondary)] bg-[var(--bg-secondary)] p-2 rounded border">
                    {analysis.notes}
                  </p>
                )}
