...
export function SkillGapAnalysisPanel() {
  ...
  const getPriorityColor = (priority: SkillGap['priority']) => {
    switch (priority) {
      ...
      default:
        return 'bg-[var(--bg-tertiary)] text-[var(--text-secondary)] border-[hsl(var(--border))]';
    }
  };
  ...
  const getPriorityIcon = (priority: SkillGap['priority']) => {
    switch (priority) {
      ...
      default:
        return <Target className="w-4 h-4 text-[var(--text-secondary)]" />;
    }
  };
      ...
      <Card className="p-12 text-center">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <h3 className="text-xl font-semibold text-[var(--text-primary)]">No Skills to Analyze</h3>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="space-y-2">
          <h2 className="text-2xl font-bold text-[var(--text-primary)]">Skill Gap Analysis</h2>
          <p className="text-[var(--text-secondary)]">Identify areas where you need to improve to reach your target levels</p>
        </div>
                      ...
                      <Card className="hover:shadow-lg transition-all duration-200">
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between text-sm">
                              <span className="text-[var(--text-secondary)]">Target Level</span>
                              <span className="font-medium">Level {gap.targetLevel}</span>
                            </div>
                            
                            <div className="space-y-2">
                              <div className="flex items-center justify-between text-sm">
                                <span className="text-[var(--text-secondary)]">Progress Needed</span>
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm font-medium text-[var(--text-primary)]">Recommendations</span>
                            </div>
                            
                            <div className="space-y-2">
                              {gap.recommendations.slice(0, 3).map((recommendation, idx) => (
                                <div key={idx} className="flex items-start gap-2 text-sm text-[var(--text-secondary)] p-2 bg-[var(--bg-tertiary)] rounded-lg">
                                  <div className="w-1.5 h-1.5 bg-[var(--accent-primary)] rounded-full mt-2 flex-shrink-0" />
