    ...
    return (
      <Card className="p-6">
        <div className="text-center text-[var(--text-tertiary)]">
          <Briefcase className="h-12 w-12 mx-auto mb-4 text-[var(--text-tertiary)]" />
    ...
    <Card className="p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-lg font-semibold text-[var(--text-primary)]">Job Market Analysis</h3>
          {jobData?.trending && (
      ...
      {isLoading ? (
        <div className="text-center py-8">
          ...
          <p className="text-[var(--text-secondary)]">Analyzing job market...</p>
        </div>
      ) : jobData ? (
        <div className="space-y-6">
          {/* Market Overview */}
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center p-4 bg-[var(--accent-primary)]/10 rounded-lg">
              <div className="text-2xl font-bold text-[var(--accent-primary)]">{jobData.totalJobs}</div>
          ...
          {jobData.salaryRange && (
            <div className="p-4 bg-[var(--bg-tertiary)] rounded-lg">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-sm font-medium text-[var(--text-secondary)]">Salary Range</span>
              </div>
          ...
          {jobData.topLocations.length > 0 && (
            <div>
              <div className="flex items-center space-x-2 mb-3">
                ...
                <span className="text-sm font-medium text-[var(--text-secondary)]">Top Locations</span>
              </div>
              <div className="space-y-2">
                {jobData.topLocations.map((location, index) => (
                  <div key={index} className="flex justify-between items-center text-sm">
                    <span className="text-[var(--text-secondary)]">{location.location}</span>
                    <span className="text-[var(--text-tertiary)]">{location.count} jobs</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {jobData.recentJobs.length > 0 && (
            <div>
              <h4 className="text-sm font-medium text-[var(--text-secondary)] mb-3">Recent Listings</h4>
              <div className="space-y-3">
                {jobData.recentJobs.map((job, index) => (
                  <div key={index} className="p-3 border border-[hsl(var(--border))] rounded-lg">
                    <div className="font-medium text-sm text-[var(--text-primary)] mb-1">{job.title}</div>
                    ...
                    <div className="text-xs text-[var(--text-tertiary)] mt-1">{job.postedDate}</div>
                  </div>
                ))}
              </div>
            </div>
