...
export function AIAssistant() {
  ...
  return (
    <>
      {!open && (
        <button
          className="fixed bottom-5 right-5 z-50 w-14 h-14 rounded-full bg-[var(--accent-primary)] text-white shadow-lg hover:bg-[var(--accent-primary)]/90"
      ...
      {open && (
        <div className="fixed bottom-5 right-5 z-50 w-[360px] max-w-[92vw] h-[540px] bg-[var(--bg-secondary)] rounded-xl shadow-2xl border flex flex-col">
          <div className="px-3 py-2 border-b flex items-center justify-between">
            <div className="text-sm font-semibold">Career AI Assistant</div>
            <button className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)]" onClick={() => setOpen(false)} aria-label="Close">✕</button>
          </div>
          ...
          <div className="flex-1 overflow-auto p-3 space-y-3">
            {messages.length === 0 && (
              <div className="text-sm text-[var(--text-secondary)]">Ask about concepts (APO, Sharpe), features (how-tos), or your results.</div>
            )}
            {messages.map((m, i) => (
              <div key={i} className={`text-sm rounded-md p-2 ${m.role === 'user' ? 'bg-[var(--accent-primary)]/10 self-end' : 'bg-[var(--bg-tertiary)]'}`}>{m.content}</div>
            ))}
          </div>
          <div className="p-3 border-t">
            <div className="flex gap-2">
              ...
              />
              <button
                className="px-3 py-2 text-sm rounded-md bg-[var(--accent-primary)] text-white disabled:opacity-50"
                onClick={() => send()}
