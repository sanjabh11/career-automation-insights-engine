...
export function SkillRecommendationsPanel({ occupationTitle, occupationCode }: SkillRecommendationsProps) {
  ...
  const getPriorityLabel = (priority: number) => {
    switch (priority) {
      ...
      default: return { label: 'Medium', color: 'bg-[var(--accent-primary)]/20 text-[var(--accent-primary)]' };
    }
  };
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {recommendations.map((rec, index) => {
                ...
                return (
                  <div key={index} className="p-4 border rounded-lg bg-[var(--bg-tertiary)]">
                    ...
                    <p className="text-sm text-[var(--text-secondary)] mb-3">{rec.explanation}</p>
            ...
            <div className="space-y-2">
              {resources.map((resource, index) => (
                <a 
                  ...
                  className="p-3 border rounded-lg flex items-start gap-3 hover:bg-[var(--bg-tertiary)] transition-colors"
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h4 className="font-medium text-[var(--accent-primary)]">{resource.title}</h4>
                      <Badge variant="outline" className="text-xs">
                        {resource.cost_type}
                      </Badge>
                    </div>
                    <p className="text-xs text-[var(--text-secondary)] mt-1">Provider: {resource.provider}</p>
                    {resource.description && (
                      <p className="text-sm text-[var(--text-secondary)] mt-2">{resource.description}</p>
                    )}
                  </div>
                  <ExternalLink className="w-4 h-4 text-[var(--text-tertiary)] flex-shrink-0 mt-1" />
                </a>
              ))}
            </div>
          </div>
