...
export function AIImpactDashboard() {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      default:
        return <Info className="w-4 h-4 text-[var(--accent-primary)]" />;
    }
  };
          ...
          <CardContent className="space-y-4">
            ...
            <div className="relative">
              ...
              {occupations.length > 0 && (
                <div className="absolute z-10 mt-1 w-full bg-[var(--bg-secondary)] border rounded-md shadow-lg max-h-60 overflow-y-auto">
                  {occupations.map((occ) => (
                    <div
                      key={occ.code}
                      className="p-3 hover:bg-[var(--bg-hover)] cursor-pointer border-b last:border-b-0"
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="tasks" className="space-y-4">
              {isLoadingTasks ? (
                          ...
                          {categoryTasks.length > 0 ? (
                            categoryTasks.map((task, index) => (
                              <div key={index} className="p-3 bg-[var(--bg-tertiary)] rounded-lg text-sm">
                                ...
                                {task.explanation && (
                                  <p className="text-xs text-[var(--text-secondary)] mt-1">{task.explanation}</p>
                                )}
                ...
                <div className="text-center p-8 bg-[var(--bg-tertiary)] rounded-lg">
        ...
        <motion.div variants={itemVariants}>
          <Card className="bg-[var(--bg-tertiary)]">
            <CardContent className="p-8 text-center">
              <Info className="w-12 h-12 text-[var(--accent-primary)] mx-auto mb-4" />
