...
export function OutcomesList() {
  ...
  return (
    <Card className="p-4">
      ...
      {!user ? (
        <div className="text-xs text-[var(--text-secondary)]">Sign in to view your outcomes.</div>
      ) : loading ? (
        <div className="py-2"><LoadingSpinner size="sm" /></div>
      ) : error ? (
        <div className="text-xs text-red-600">{error}</div>
      ) : rows.length === 0 ? (
        <div className="text-xs text-[var(--text-secondary)]">No outcomes yet. Submit your first one above.</div>
      ) : (
