    ...
    <Card className="p-6">
      ...
      <div className="space-y-3">
        {selectedJobs.map((job) => {
          ...
          return (
            <div key={job.code} className="p-3 bg-[var(--bg-tertiary)] rounded-lg">
              <div className="flex items-center justify-between mb-2">
                <div className="flex-1">
                  <p className="font-medium text-sm text-[var(--text-primary)]">{job.title}</p>
                  <p className="text-xs text-[var(--text-tertiary)]">{job.code}</p>
                </div>
              ...
              {job.timeline && (
                <Badge className="text-xs bg-[var(--accent-primary)]/10 text-[var(--accent-primary)]">
                  {job.timeline}
                </Badge>
              )}
            </div>
