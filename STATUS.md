...
| `npm run verify:live-gate-evidence` | Pass | Confirmed the redacted evidence intake verifier is wired for credential-gated live proof, rejects future-dated proof metadata, rejects item dates later than `asOf`, and no local evidence file is currently attached. |
| `npm run verify:commercial-evidence-records` | Pass | Confirmed the redacted commercial evidence records verifier is wired, requires unique partner/outcome hashes, rejects future-dated proof metadata, rejects record dates later than `asOf`, and no local partner/outcome evidence file is currently attached. A passing partner/outcome proof still requires owner-held permissioned records. |
...
| `npm run verify:remediation-gates` | Pass | Wrote the non-mutating external gate ledger. Current result is `goalComplete=false` because Stripe test-mode checkout, production calibration, authenticated live e2e, live MRR, partners, and outcomes still require owner/live evidence. The ledger consumes valid redacted commercial evidence records when present. |
