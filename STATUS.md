...
| `npm run verify:repo-presentation:github` | Pass | Confirms the GitHub API description is visible, the Phase E branch exposes `LICENSE` through the contents API, and the GitHub API `license` field remains default-branch-pending until merge. |
...
| `npm run verify:owner-evidence-prep` | Pass as incomplete prep status | Reports missing owner-held env names, local intake files, and source proof artifacts without printing secrets. Use `npm run prepare:owner-evidence -- --write` to create ignored local scaffolding, then fill values owner-side. |
| `npm run verify:owner-evidence-fixtures` | Pass | Confirms temporary synthetic non-secret live/commercial evidence fixtures can satisfy the validators and final remediation gate with `goalCompleteWithSyntheticFixtures=true`; this proves schema compatibility only and is not live or commercial proof. |
| `npm run verify:owner-evidence-closeout` | Pass as incomplete status bundle | Runs the ordered owner-proof closeout path without writing: live-evidence compose, commercial-record compose, live verifier, commercial verifier, and fail-closed remediation gates. It exits successfully only because `--allow-incomplete` is part of this status script; `npm run closeout:owner-evidence` remains fail-closed until owner evidence is attached. |
| `npm run verify:remediation-completion-audit` | Pass | Generates the phase-by-phase closeout ledger with PR summaries, key files, commands, acceptance evidence, confidence deltas, and remaining external gates while preserving `goalComplete=false` until owner-held evidence is attached. |
...
| `npm run verify:remediation-gates` | Pass | Ran the external gate ledger in read-only mode. Current result is `goalComplete=false` because Stripe test-mode checkout, production calibration, authenticated live e2e, live MRR, partners, and outcomes still require owner/live evidence. Use `npm run verify:remediation-gates:write` or `npm run verify:commercial` when tracked ledger artifacts should be refreshed. |
...
- `docs/commercialization/live-gate-evidence-template.json` and `npm run verify:live-gate-evidence` define a redacted evidence intake path for owner-held credential-gated live proof: Stripe test checkout, production calibration, authenticated live artifact e2e, and live MRR. The remediation ledger consumes the default local evidence file, `LIVE_GATE_EVIDENCE_PATH`, or `--live-evidence <path>`. The default local evidence file is git-ignored and must not contain secrets, raw customer data, partner contact details, private outcome notes, future-dated proof metadata, or evidence item dates later than `asOf`. `npm run verify:live-gate-evidence -- --require-complete` fails closed unless all four credential-gated proof items are accepted.
- `npm run compose:live-gate-evidence -- --write --require-complete` composes the gitignored redacted live evidence file from passed owner-run proof artifacts only. It fails closed when any proof artifact is missing, skipped, failed, or lacks required summary fields; it does not make skipped artifacts count as proof.
- `docs/commercialization/commercial-evidence-records-template.json` and `npm run verify:commercial-evidence-records` define a redacted record path for partner commitments and documented outcomes. The remediation ledger consumes the default local records file, `COMMERCIAL_EVIDENCE_RECORDS_PATH`, or `--commercial-evidence <path>` and only upgrades the partner/outcome gates when accepted unique hashes satisfy the gate. The default local records file is git-ignored and must not contain partner names, contact details, contracts, raw notes, private quotes, customer data, future-dated proof metadata, or record dates later than `asOf`.
- `docs/commercialization/commercial-evidence-intake-template.json` and `npm run compose:commercial-evidence-records -- --write --require-all` define a gitignored owner-intake path that hashes partner/outcome refs with an owner-held salt, writes only redacted evidence records, and validates the output before writing. The intake file and salt must stay outside tracked files.
- `npm run verify:owner-evidence-prep` and `npm run prepare:owner-evidence -- --write` provide a non-proof local setup path for the owner-held evidence workflow. The helper can create ignored `.env.local` and commercial-intake placeholders if absent, but placeholders and unloaded env keys never satisfy final gates.
- `npm run verify:owner-evidence-fixtures` creates temporary synthetic, non-secret redacted evidence files outside the repository and confirms the same live-gate, commercial-evidence, and final remediation-gate validators can reach `goalComplete=true` when all required fields are present. This is a schema-path smoke test only; it does not count as live checkout, production calibration, live MRR, partner commitment, or outcome evidence.
- `npm run closeout:owner-evidence -- --write --refresh-tracked` is the final owner-side closeout bundle after live proof artifacts and commercial intake exist. It composes `live-gate-evidence.local.json`, composes `commercial-evidence-records.local.json`, validates both fail-closed, runs `verify-remediation-gates --require-complete`, and refreshes tracked closeout artifacts only when all required redacted evidence is accepted. Use `npm run verify:owner-evidence-closeout` for a non-writing status run that is allowed to remain incomplete.
- `npm run verify:remediation-completion-audit:write` writes `docs/commercialization/remediation-completion-audit-latest.json` and `.md` from the current gate ledger, keeping phase deliverables, confidence deltas, and remaining external gates current after verifier changes.
...
- `docs/commercialization/remediation-external-gates-latest.md` is the current generated ledger for the remaining external gates. It records secret presence by variable name only, reads only redacted evidence metadata when present, and does not apply migrations, deploy functions, create Stripe sessions, or query live customer data. Refresh it with `npm run verify:remediation-gates:write` or `npm run verify:commercial`.
...
- Phase C runtime smoke is wired into `.github/workflows/phase-c-runtime-smoke.yml` for pull requests and pushes across `live-auth-e2e-closeout` and the Phase A-E remediation branch chain, including `phase-e-commercial-validation`.
...
- `docs/commercialization/live-gate-evidence-template.json`, `scripts/lib/liveGateEvidence.mjs`, and `npm run verify:live-gate-evidence` provide a redacted owner-evidence schema for the remaining credential-gated live proof gates. Future-dated `asOf` or `observedAt` metadata is rejected, and `observedAt` must not be later than `asOf`. `npm run verify:live-gate-evidence -- --require-complete` fails closed unless all four credential-gated proof items are accepted. `npm run verify:remediation-gates -- --live-evidence <path>` can consume an explicit owner-held redacted file path for the final ledger. The current repo has no attached accepted live evidence file.
- `scripts/compose-live-gate-evidence.mjs` can convert passed `stripe-test-checkout`, `production-calibration`, `commercial-live-auth-e2e`, and `stripe-live-mrr` proof artifacts into the gitignored live-gate evidence file, then validates the composed file with the shared live-gate validator before writing. Current local source artifacts are skipped planning artifacts, so the composer correctly refuses to write proof.
- `docs/commercialization/commercial-evidence-records-template.json`, `scripts/verify-commercial-evidence-records.mjs`, and `npm run verify:commercial-evidence-records` provide a redacted owner-record schema for committed design partners and documented outcomes. Future-dated `asOf`, `committedAt`, or `observedAt` metadata is rejected, and record dates must not be later than `asOf`. `npm run verify:remediation-gates -- --commercial-evidence <path>` consumes explicit records and can mark the partner/outcome gates as externally proven when the records pass with unique partner/outcome hashes. The current artifact is `no_local_evidence`, not partner or outcome proof.
- `scripts/compose-commercial-evidence-records.mjs` can compose the gitignored commercial evidence records file from local owner intake without printing raw refs or salts. Current local state has no owner intake file, so it correctly refuses to write records.
...
- `npm run verify:remediation-gates` now summarizes the entire A-E remediation boundary without dirtying tracked artifacts by default; `npm run verify:remediation-gates:write` writes `docs/commercialization/remediation-external-gates-latest.json` plus `.md`.
- `.github/workflows/phase-c-runtime-smoke.yml` and `.github/workflows/commercial-proof-pack.yml` now include `live-auth-e2e-closeout` plus the Phase A-E remediation branch chain for push/PR CI coverage. Hosted green-run evidence must still be inspected after each workflow-affecting push before claiming CI-green status.
