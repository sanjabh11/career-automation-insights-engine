...
| `npm run verify:live-gate-evidence` | Pass | Confirmed the redacted evidence intake verifier is wired for credential-gated live proof and no local evidence file is currently attached. |
| `npm run verify:commercial-evidence-records` | Pass | Confirmed the redacted commercial evidence records verifier is wired, requires unique partner/outcome hashes, and no local partner/outcome evidence file is currently attached. A passing partner/outcome proof still requires owner-held permissioned records. |
...
- `docs/commercialization/live-gate-evidence-template.json`, `scripts/lib/liveGateEvidence.mjs`, and `npm run verify:live-gate-evidence` provide a redacted owner-evidence schema for the remaining credential-gated live proof gates. The current repo has no attached accepted live evidence file.
- `docs/commercialization/commercial-evidence-records-template.json`, `scripts/verify-commercial-evidence-records.mjs`, and `npm run verify:commercial-evidence-records` provide a redacted owner-record schema for committed design partners and documented outcomes. `npm run verify:remediation-gates` consumes those records and can mark the partner/outcome gates as externally proven when the records pass with unique partner/outcome hashes. The current artifact is `no_local_evidence`, not partner or outcome proof.
