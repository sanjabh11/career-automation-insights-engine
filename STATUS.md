...
| `npm run lint` | Fail | 21 explicit-`any` errors and 31 React warnings remain after excluding inactive backups/archives, clearing mechanical lint errors, and type-hardening the runtime env bootstrap, Netlify APO/O*NET proxy event boundaries, Career Planning job-market response handling, header validation-metric query boundary, dashboard saved-analysis load contract, assistant function response/error handling, outcomes list row/error handling, selected-career props, organization/search-history/subscription hook error and input boundaries, econ sync runtime env rows, API error normalization, web-vitals compatibility/persistence, stat-card icon typing, AI Impact Dashboard search parsing, Career Simulator result/risk handling, AI Impact Planner, Outcomes KPI, calculate-APO, Industry dashboard, Validation calibration, Job Zones, Job Zone Ladders query row typing, Quality web vitals query typing, Technology Skills query result typing, course-search SerpAPI result typing, counselor report HTML contract typing, APO UI selection/explanation boundaries, Skill Adjacency Graph, O*NET enrichment, Whop webhook, roadmap-generation, task-analysis, BLS sync, heatmap snapshot, skill recommendation, intelligent task-assessment test, personalized skill-recommendation test, Bridge Role Pathway, Ecosystem Risk Card, Premium Report Summary, O*NET enrichment hook, saved-analyses unified hook, saved-analyses remote hook, Occupation Detail, Operations telemetry, export utility, explain-APO, find-bridge-roles, generate-learning-path, search-tasks, manage-context, search-occupations, serpapi-jobs, useSkills error paths, Crosswalk Wizard payload handling, Outcome Survey payload/form handling, useAlerts error handling, useSavedSelections JSON/error handling, Bootcamp Dashboard query payload handling, Compare Page saved-analysis item typing, Saved Analyses Panel callback/share types, Market Map tooltip/invoke typing, shared-analysis RPC payload typing, Workshop Booking select callback typing, AI career coach function/test response boundaries, market-intelligence job-data/analysis parsing, shared SerpAPI client result typing, and serpapi-jobs response typing. |
